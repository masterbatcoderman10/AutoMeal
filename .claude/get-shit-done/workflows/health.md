...
```bash
# SDK resolution: prefer project-local runtime bundles before global gsd-sdk
GSD_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
if [ -n "${RUNTIME_DIR:-}" ]; then
  GSD_TOOLS="$RUNTIME_DIR/get-shit-done/bin/gsd-tools.cjs"
elif [ -f "$GSD_ROOT/.codex/get-shit-done/bin/gsd-tools.cjs" ]; then
...
TASKS_DIR="/Users/mali/Documents/Projects/MealTracker/.claude/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/mali/Documents/Projects/MealTracker/.claude/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/mali/Documents/Projects/MealTracker/.claude/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
