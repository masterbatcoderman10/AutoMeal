...
1. Run calibration:
   - Command: `rtk .venv/bin/python scripts/embed_match_smoke.py --mode calibrate --sample sample_images/IMG_4583.HEIC --same-food-peer sample_images/IMG_4583.HEIC --cross-modal-text "rice and lentils" --random-food sample_images/IMG_4611.HEIC`
2. Expected:
...
4. Note:
   - The checked-in sample set currently proves self-similarity with `IMG_4583.HEIC`; replace `--same-food-peer` with a true second photo of the same dish when collecting final human evidence.

## 2) Same-food re-photo match behavior (D-08, D-09)

1. Run seed-demo first to ensure a known food visual exists:
   - Command: `rtk .venv/bin/python scripts/embed_match_smoke.py --mode seed-demo --sample sample_images/IMG_4583.HEIC --database-url "$DATABASE_URL"`
2. Run repeat confirmation for the same seed image:
   - Command: `rtk .venv/bin/python scripts/embed_match_smoke.py --mode repeat-confirmation --sample sample_images/IMG_4583.HEIC --database-url "$DATABASE_URL"`
3. Expected:
...
1. Command: `rtk .venv/bin/python scripts/embed_match_smoke.py --mode unresolved-probe --sample sample_images/IMG_4583.HEIC --database-url "$DATABASE_URL"`
2. Expected:
...
1. Run:
   - `rtk .venv/bin/python scripts/embed_match_smoke.py --mode repeat-confirmation --sample sample_images/IMG_4583.HEIC --database-url "$DATABASE_URL"`
2. Expected:
