...
EXPECTED_FAILURES = {
    "tests.test_bot_contract.HandlerTests.test_meal_interview_text_fails_closed_when_multiple_meal_sessions_are_ambiguous",
    "tests.test_bot_contract.HandlerTests.test_meal_interview_text_routes_reply_to_matching_prompt_message_id",
}
...
def main() -> None:
    ...
    if not result.failure_ids:
        raise SystemExit("RED harness expected interview routing failures, but the suite passed.")
    ...
    print("RED harness passed: expected concurrent interview routing failures are present.")
    if output:
        print(output)
